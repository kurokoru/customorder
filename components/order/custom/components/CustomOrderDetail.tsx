...
// CustomOrderDetail component
export default function CustomOrderDetail({
  data,
  transactionId,
  setTransactionId,
}: CustomOrderDetailProps) {
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: 'Custom Order Receipt',
    onBeforeGetContent: () => {
      setPrinting(true);
    },
    onAfterPrint: () => {
  ...
  const handleCheckout = async () => {
    setLoading(true);
    try {
      const response = await axios.patch(`/api/custom-transactions/${transactionId}`, {
      ...
      eventBus.emit('clearCustomOrderData');
    } catch (error) {
